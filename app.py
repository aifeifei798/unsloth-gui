"""
=====================================================================================
🚀 Unsloth GUI Trainer & Playground (v3.7 - 极简优化版) 🚀
=====================================================================================
此版本根据“更少输入，更易成功”的核心原则进行了优化：
1.  移除了优化器选择下拉框，将 adamw_8bit 作为内置默认优化器。
    这简化了界面，并为消费级显卡用户提供了最佳实践，避免了因选择错误
    的优化器导致的内存溢出问题。
"""
...
def prepare_dataset(dataset_config):
    if dataset_config.get("is_local", False):
        ...
        if not Path(dataset_path).exists(): raise FileNotFoundError(f"本地数据集路径未找到: {dataset_path}")
...
def train_model(
    ...
    args = {
        ...
        "learning_rate": float(lr), "logging_steps": 1,
        # vvvvvvvvvvvvvv 这是修改的地方 vvvvvvvvvvvvvv
        "optim": "adamw_8bit", # 固定使用 adamw_8bit 优化器
        # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
def load_inference_model(base_model_name, lora_name, progress=gr.Progress(track_tqdm=True)):
    ...
    if not base_model_name or not lora_name: return None, None, "错误：必须同时选择一个基础模型和一个 LoRA 适配器。"
    model_config = next((item for item in MODELS_CONFIG if item["display_name"] == base_model_name), None)
    if not model_config: return None, None, f"错误：找不到基础模型 '{base_model_name}' 的配置。"
    base_model_id = model_config['model_id']
    lora_path = Path(OUTPUTS_PARENT_DIR) / lora_name
    if not lora_path.exists(): return None, None, f"错误：LoRA 目录未找到: {lora_path}"
    try:
        progress(0.2, desc=f"正在加载基础模型: {base_model_id}...")
        model, tokenizer = FastLanguageModel.from_pretrained(model_name=base_model_id, max_seq_length=2048, load_in_4bit=True)
...
def update_training_mode_ui(mode):
    ...
    return {num_epochs_slider: gr.update(visible=is_epoch_mode), max_steps_slider: gr.update(visible=not is_epoch_mode), save_steps_input: gr.update(visible=not is_epoch_mode)}

with gr.Blocks(theme=gr.themes.Soft(), css="footer {display: none !important}") as demo:
    gr.Markdown("# 🚀 Unsloth GUI Trainer & Playground (v3.7)")
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    with gr.Accordion("4. 训练核心参数 (8GB 优化)", open=True):
                        ...
                        # vvvvvvvvvvvvvv 这是修改的地方 vvvvvvvvvvvvvv
                        # optimizer_dropdown = gr.Dropdown(["adamw_8bit", "adamw_torch"], value="adamw_8bit", label="优化器") # 已移除
                        # ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    ...
    train_inputs = [
        ...
        batch_size_slider, grad_accum_slider, learning_rate_slider # <-- 已移除 optimizer_dropdown
    ]
